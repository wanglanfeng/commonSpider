...
// model 基本操作
PROJECT.ModelBase = {
    create: function (url, action_data, callback) {
        var data_json = {
            action: 'create',
            action_data: JSON.stringify(action_data)
        };
        PROJECT.ajax(url, data_json, function (res) {
    ...
    update: function (url, data, callback) {
        ...
        $.extend(data_json, data);
        data_json['action_data'] = JSON.stringify(data_json['action_data']);
        PROJECT.ajax(url, data_json, function (res) {
            callback && callback(res);
        });
    },
    delete: function (url, data, callback) {
        if (data == undefined || data.length == 0) {
            PROJECT.tool.warnAlert('至少选择一条记录删除！');
            return false;
        }
        PROJECT.tool.dangerConfirm('是否确认删除？', function () {
            data['action'] = 'delete';
            PROJECT.ajax(url, data, function (res) {
                callback && callback(res);
            });
        });
    },
    deleteById: function (url, ids, callback) {
...
PROJECT.BTableBase = {
    deleteBatch: function (table) {
        var ids = [];
        var rows = table.func('getAllSelections');
        for (var row in rows) {
            rows[row]['id'] && ids.push(rows[row]['id']);
        }
        PROJECT.ModelBase.deleteById(table.context.config.url, ids, function (res) {
            if (res.status) {
                table.refresh();
                toastr.success('删除成功');
            }else{
